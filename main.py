...
"LocationAbbr" represents the abbreviations of each state, and "YearStart" indicates the year when data collection began. By using Variance Inflation Factor (VIF) analysis, we verified that "DataValue_dia02," "DataValue_dia03," and "DataValue_dia04" each had a VIF value less than 5, suggesting negligible multicollinearity among these variables. This finding justified the inclusion of these three continuous variables, along with "YearStart" and "LocationAbbr," as predictors in our model.
    ...
    html.Div([
        ...
        html.P("""
               Overall, We explored six regression models: Linear Regression, Random Forest, Ridge Regression, Lasso Regression, Support Vector Machine (SVM), and Decision Tree,utilized R-square and Mean Squared Error (MSE) to evaluate these models.
               We ended up choosing the simple linear regression model as our final model, as it has the highest R-square and lowest MSE. This model is also the most interpretable, which is important for our study as we want to understand the impact of different predictors on the percentage of diabetes patients among adults.
            In terms of interpretation of the model, in our final linear model, we have found that the p-value for all the dummy variables of LocationAbbr, representing states, are less than 0.05, indicating that they are statistically significant. Therefore, we can see that location, or states, in this case, is most significant in predicting the percentage of Diabetes patients among adults.
            This is in-line with our Exploratory Data Analysis, as we discovered that the percentage of diabetes patients among adults differ a lot between states. Specifically, deep south states like Mississippi and Alabama seem to have a higher percentage of diabetes patients among adults than other places in America, and West Virginia has the highest percentage of diabetes patients among all the states in America.
            While we didn't use GDP per capita as a predictor in our final model, we can see from our EDA that states with higher GDP per capita, like New York, California, and Washington, have lower percentage of diabetes patients among adults than states with lower GDP per capita, like West Virginia, Mississippi, and Alabama.
            This is a very interesting finding, as it suggests that the wealth of a state might have an impact on the percentage of diabetes patients among adults, and we can further investigate this relationship in future studies.
""")
    ]),
